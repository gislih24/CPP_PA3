...
- [ ] Your program should be able to read simple expressions containing positive integers, addition, multiplication, and parentheses. 
...
For example, consider the expression “`2+7*(1+3+4*2)`”.
- Evaluation can begin by simplifying either `1+3` to `4` or `4*2` to `8`.
- Subsequently, `4+8` is simplified to `12`, and the parentheses are removed.
- Next, `7 * 12` is evaluated, followed by the final addition.
...
If your input string is `buf` and you implement the AST as literal nodes with pointers to each other, the main parsing function in your program might be something like “`Node parse(string &s, int left, int right)`”.
Calling `parse(s, 0, 10)` would then parse the first 11 characters of parse.
If you decide to split on an operator at index 7, you could then evaluate what is to the left and right of it with `parse(s, 0, 6)` and `parse(8, 10)`.
...
- [ ] **Variables (15%):** The input can contain variables (tokens consisting of lower-case ASCII characters).
  - [ ] These have to become part of the AST (so a leaf could contain “`x`” instead of say “`4`”).
  ...
  - [ ] Each line of that input file contains one variable value, given in the format “`x=7`”.
- [ ] **AST (Abstract Syntax Tree) optimisations (~10%):** This is only relevant if you have already implemented variables.
  ...
  - [ ] It can mean evaluating constants and simplifying them, replacing things like `0*x` by simply `0`, even though `x` is a variable and so on.
